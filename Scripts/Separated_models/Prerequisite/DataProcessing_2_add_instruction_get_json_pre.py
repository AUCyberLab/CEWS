...
def convert_table_to_finetuning_json(input_file_path, output_json_path):
    """
    Read a table containing CVE and CAPEC Prerequisites data and convert it
    into the JSON format required for LLM fine-tuning.
    """
    ...
    for _, row in df.iterrows():
        prereq = row.get('CAPEC_Prerequisites', '')

        input_text = (
            f"Target CAPEC:\n"
            f"CAPEC-ID: {row.get('CAPEC-ID', '')}\n"
            f"CAPEC Name: {row.get('CAPEC_Name', '')}\n"
            f"CAPEC Prerequisites: {prereq}\n\n"
...
if __name__ == "__main__":
    input_file = 'data/Early_late_dataset/Prerequisite_train.xlsx'
    output_file = 'data/Early_late_dataset/Prerequisite_train.json'

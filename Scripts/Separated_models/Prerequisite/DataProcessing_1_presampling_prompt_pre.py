...
def process_and_sample(
    ...
    for cve_id, group in cve_grouped:
        ...
        for _, row in group.iterrows():
            ...
            sampled_data.append({
                ...
                'CAPEC_Prerequisites': row.get('CAPEC_Prerequisites', ''),
                'Label': 1,
                'Expected_Score': "1.0",
            ...
            if global_candidates:
                ...
                for neg_cid in random.sample(global_candidates, k):
                    ...
                    sampled_data.append({
                        ...
                        'CAPEC_Prerequisites': neg_info['CAPEC_Prerequisites'],
                        'Label': 0,
                        'Expected_Score': "0.0",
    ...
    for index, row in final_df.iterrows():
        ...
        txt_lines.append(f"- Target CAPEC Prerequisites: {row['CAPEC_Prerequisites']}\n")
...
def build_dataset_pipeline(
    ...
    if 'CAPEC_Prerequisites' in df.columns:
        df['CAPEC_Prerequisites'] = df['CAPEC_Prerequisites'].fillna('')
    ...
    if 'CAPEC-ID' in full_capec_df.columns:
        id_col = 'CAPEC-ID'
    elif 'ID' in full_capec_df.columns:
        id_col = 'ID'
    else:
        raise KeyError("Full CAPEC dictionary must contain a 'CAPEC-ID' or 'ID' column.")

    has_prereq = 'Prerequisites' in full_capec_df.columns
    ...
    for _, row in full_capec_df.iterrows():
        ...
        if cid in seen_capecs or pd.isna(row['Name']):
            continue
        prereq_val = row['Prerequisites'] if has_prereq and pd.notna(row['Prerequisites']) else ""
        ood_capec_dict[cid] = {
            'CAPEC_Name': row['Name'],
            'CAPEC_Prerequisites': prereq_val,
        }
...
if __name__ == "__main__":
    INPUT_FILE = "data/Prerequisite/Prerequisite_merged_large.xlsx"
    FULL_CAPEC_DICTIONARY_FILE = "data/Prerequisite/capec_database.xlsx"
    OUTPUT_DIRECTORY = "data/Prerequisite/Processed_Data/"
    OUTPUT_TAG = "prerequisite_large_42"
    GLOBAL_SEED = 42

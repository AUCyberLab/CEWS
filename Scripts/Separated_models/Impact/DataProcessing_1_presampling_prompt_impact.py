...
def process_and_sample(
    ...
    for index, row in final_df.iterrows():
        ...
        txt_lines.append(f"- Target CAPEC Impact: {row['Impact']}\n")
...
def build_dataset_pipeline(
    ...
    # Replace NaN Impact values with empty strings so they don't leak into prompts as "nan"
    ...
    has_impact = 'Impact' in full_capec_df.columns
    ...
    for _, row in full_capec_df.iterrows():
        ...
        if cid in seen_capecs or pd.isna(row['Name']):
            continue
        impact_val = row['Impact'] if has_impact and pd.notna(row['Impact']) else ""
        ood_capec_dict[cid] = {
            'CAPEC_Name': row['Name'],
            'Impact': impact_val,
        }
...
if __name__ == "__main__":
    INPUT_FILE = "data/Impact/Impact_merged_large.xlsx"
    FULL_CAPEC_DICTIONARY_FILE = "data/Impact/CAPEC_Impacts_Only.xlsx"
    OUTPUT_DIRECTORY = "data/Impact/Processed_Data/"
    OUTPUT_TAG = "impact_large_42"
    GLOBAL_SEED = 42

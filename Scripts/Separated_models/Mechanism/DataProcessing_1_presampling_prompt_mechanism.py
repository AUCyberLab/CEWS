    ...
    for cve_id, group in cve_grouped:
        ...
        for _, row in group.iterrows():
            ...
            sampled_data.append({
                ...
                'CAPEC_Mechanism': row.get('CAPEC_Mechanism', ''),
                'Label': 1,
                'Expected_Score': "1.0",
...
def build_dataset_pipeline(
    ...
    if 'CAPEC_Mechanism' in df.columns:
        df['CAPEC_Mechanism'] = df['CAPEC_Mechanism'].fillna('')
    ...
    # Tolerate either 'CAPEC-ID' or 'ID' as the identifier column
    ...
    has_mechanism = 'Mechanism' in full_capec_df.columns
    ...
    for _, row in full_capec_df.iterrows():
        ...
        if cid in seen_capecs or pd.isna(row['Name']):
            continue
        mechanism_val = row['Mechanism'] if has_mechanism and pd.notna(row['Mechanism']) else ""
        ood_capec_dict[cid] = {
            'CAPEC_Name': row['Name'],
            'CAPEC_Mechanism': mechanism_val,
        }
...
if __name__ == "__main__":
    INPUT_FILE = "data/Mechanism/Mechanism_merged_small.xlsx"
    FULL_CAPEC_DICTIONARY_FILE = "data/Mechanism/CAPEC_parsed_with_level_and_mechanism.xlsx"
    OUTPUT_DIRECTORY = "data/Mechanism/Processed_Data/"
    OUTPUT_TAG = "mechanism_small_42"
    GLOBAL_SEED = 42
